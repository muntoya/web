...
function getInfo(){
	...
	if ( selectedNum != 1){
		...
		var url='';
		if ($('#lun_detail').hasClass('choose')) {
			url = '/api/resource/pages/lun/' + id + '/detail';
		}else if ($('#lun_snapshot').hasClass('choose')){
			url = '/api/resource/pages/lun/' + id + '/snapshot';
		}
		$.ajax({
	        type: 'GET',
	        url: url,        
...
function refreshTableData(){	
	...
	$.ajax({
        type: 'GET',
        url: '/api/resource/pages/lun/list',        
        success: function(respond) {
